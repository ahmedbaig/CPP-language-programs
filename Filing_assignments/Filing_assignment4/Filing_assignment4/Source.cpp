...
int start(string input){
	...
	else if (choice == "write" || choice == "Write" || choice == "WRITE" || choice == "w" || choice == "new" || choice == "New" || choice == "NEW" || choice == "n"){
		file_maker(input);
	}
	else{
		cout << "\nwrong input.\n";
...
int file_reader(string output){
	...
	cout << "\n\nDo you want to start a word count or do you want to go back home? \n[Enter word for word count or home to enter home. Enter no if you want to exit]: ";
	cin >> choice;
	if (choice == "home" || choice == "Home" || choice == "HOME" || choice == "h"){
		start(output);
	}
	else if (choice == "word" || choice == "Word" || choice == "WORD" || choice == "w"){
		word_count(file_name, output);
	}
	else if (choice == "no" || choice == "No" || choice == "NO" || choice == "n"){
		exit(1);
	}
...
int word_count(string file_name, string output){
	
	ifstream file;
	file.open(file_name,ios::in | ios::trunc);
	getline(file, output);
	file.close();

	cout << "\n\nThere are a total of " << output.length() << " words in this file" << endl;
